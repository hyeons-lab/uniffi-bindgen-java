...
`uniffi-bindgen-java` is currently unstable. It was originally developed by IronCore Labs to target features required by [`ironcore-alloy`](https://github.com/IronCoreLabs/ironcore-alloy/), and work is underway in the Hyeons' Lab fork with the goal of adding an idiomatic Kotlin backend. The major version is currently 0, and most changes are likely to bump the minor version.
